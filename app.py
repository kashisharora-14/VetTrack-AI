...
from gemini import analyze_pet_symptoms, analyze_pet_image
...
def dashboard():
    ...
    health_history = HealthHistory.query.filter(
        HealthHistory.pet_id.in_(pet_ids)
    ).order_by(HealthHistory.date.desc()).all()
...
def test_gemini():
    try:
        ...
        client = genai.Client(api_key="AIzaSyDLSVPNpvEH3Lh2lxmtiPVRY4OG-HVIy9o")  # replace with your key
...
def consultation(pet_id):
    ...
    consultation = Consultation(pet_id=pet.id, summary="")
